...
function Score(matrix: string[][], task: string[][]): number {
  if (!matrix || !task || matrix.length === 0 || task.length === 0) {
    console.log('[INFO] Score: Invalid input. Returning 0.')
    return 0
  }
  console.log('[INFO] Score: ', matrix, '-', task)
  const matrixRows = matrix.length
  const matrixCols = matrix[0].length
  console.log('[INFO] Score: matrix 行', matrixRows, matrixCols)
  const taskRows = task.length
  const taskCols = task[0].length
  console.log('[INFO] Score: task 行', taskRows, taskCols)
